...
The metadata text node is a special type of text node that contains metadata about the canvas like the format version and frontmatter attributes. Along with text node attributes, the metadata text node includes the following attributes:
...
- `styleAttributes` (optional, object) is a JSON object containing style attributes. The style attributes can contain any number of key-value pairs. The keys must be strings, and the values can be strings, numbers, booleans, or arrays. The style attributes are used to store additional styling information about the edge.
