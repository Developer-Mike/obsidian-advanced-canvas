...
<small>Version <code>1.0-1.0</code> — 2025-03-30</small>

You can find the TypeScript typings [here](1.0%20(1.0).d.ts).

> [!warning] This is a DRAFT
> Do not use this format in production. It is still being worked on and may change in the future.
...
The metadata text node is a special type of text node that contains metadata about the canvas like the format version and frontmatter attributes. It is stored as a text node to be compatible with the original JSON Canvas. If possible, this node should be hidden from the user and edited using a modal or similar. Along with text node attributes, the metadata text node includes the following attributes:

- `id` (required, string) is the unique ID for the node.
  - `metadata` is the required ID for the metadata node.
- `metadata` (required, object) is the metadata object. The metadata object contains the following attributes:
