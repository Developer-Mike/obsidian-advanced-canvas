...
const SPENT_HOURS = 150
const RECEIVED_DONATIONS = 234
...
interface PluginSettings {
  ...
  general: {
    nodeTypeOnDoubleClick: 'text' | 'file'
...
type PluginSettingsData = {
  [K in keyof Omit<PluginSettings, "general">]: PluginSettings[K] extends object ? boolean : PluginSettings[K]
} & {
  [K in keyof PluginSettings as (PluginSettings[K] extends object ? keyof PluginSettings[K] : never)]:
    PluginSettings[K] extends object ? PluginSettings[K][Extract<keyof PluginSettings[K], string>] : never
}

export const DEFAULT_SETTINGS_VALUES: PluginSettingsData = {
...
} as const satisfies any // TODO: Fix this

export default class SettingsManager {
  private plugin: AdvancedCanvasPlugin
  private settings: PluginSettingsData
  ...
  getSetting<T extends keyof PluginSettingsData>(key: T): PluginSettingsData[T] {
...
export class AdvancedCanvasPluginSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    for (const [headingId, heading] of Object.entries(SETTINGS) as [string, SettingsHeading][]) {
      this.createFeatureHeading(
        ...
        heading.description, 
        heading.infoSection,
        heading.disableToggle ? null : headingId as keyof PluginSettings
      )

      for (let [settingId, setting] of Object.entries(heading.children) as [keyof PluginSettings, Setting][]) {
  ...
  private createFeatureHeading(containerEl: HTMLElement, label: string, description: string, infoSection: string | null, settingsKey: keyof PluginSettings | null): SettingEl {
  ...
  private createTextSetting(containerEl: HTMLElement, settingId: keyof PluginSettings, setting: TextSetting) {
  ...
  private createNumberSetting(containerEl: HTMLElement, settingId: keyof PluginSettings, setting: NumberSetting) {
    new SettingEl(containerEl)
      .setName(setting.label)
      .setDesc(setting.description)
      .addText(text => text
  ...
  private createBooleanSetting(containerEl: HTMLElement, settingId: keyof PluginSettings, setting: BooleanSetting) {
    new SettingEl(containerEl)
      .setName(setting.label)
      .setDesc(setting.description)
  ...
  private createDropdownSetting(containerEl: HTMLElement, settingId: keyof PluginSettings, setting: DropdownSetting) {
    new SettingEl(containerEl)
      .setName(setting.label)
      .setDesc(setting.description)
  ...
  private createButtonSetting(containerEl: HTMLElement, settingId: keyof PluginSettings, setting: ButtonSetting) {
    new SettingEl(containerEl)
      .setName(setting.label)
      .setDesc(setting.description)
  ...
  private createStylesSetting(containerEl: HTMLElement, settingId: keyof PluginSettings, setting: StyleAttributesSetting) {
