...
export class AdvancedCanvasPluginSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    this.createFeatureHeading(
      containerEl,
      "Embed properties",
      "Modify properties of embedded Markdown files based on the outgoing edges.",
      'embedPropertiesEnabled'
    )

    new Setting(containerEl)
      .setName("Show overwrite warning")
      .setDesc("When enabled, a warning will be shown when overwriting the properties of a Markdown file.")
      .addToggle((toggle) =>
        toggle
          .setValue(this.settingsManager.getSetting('embedPropertiesShowOverwriteWarning'))
          .onChange(async (value) => await this.settingsManager.setSetting({ embedPropertiesShowOverwriteWarning: value }))
      )

    new Setting(containerEl)
      .setName("Add nondirectional edges")
      .setDesc("When enabled, a nondirectional edge also counts as an outgoing edge and thus will be added to the properties.")
      .addToggle((toggle) =>
        toggle
